...
const RightSidebar = () => {
  ...
  return (
    selectedUser && (
      ...
      >
        <div className="pt-16 flex flex-col items-center text-center px-6 text-xs font-light mx-auto">
          <img
            src={selectedUser?.profilePic || assets.avatar_icon}
            alt=""
            className="w-20 h-20 rounded-full object-cover mb-2"
          />
          <h1 className="px-10 text-xl font-medium flex items-center justify-center gap-2 mb-1 text-center">
            {onlineUsers.includes(selectedUser._id) && (
              <span className="w-2 h-2 rounded-full bg-green-500"></span>
            )}
            {selectedUser.fullName}
          </h1>
          <p className="text-xs font-light max-w-xs text-center">{selectedUser.bio}</p>
        </div>
